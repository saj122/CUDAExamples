...
__global__
void intensityKernel(unsigned char* d_out, const uchar4 *d_in, int w, int h) {
  const int c = threadIdx.x + blockDim.x * blockIdx.x;
  const int r = threadIdx.y + blockDim.y * blockIdx.y;
  if ((c >= w) || (r >= h)) return;

  d_out[r*w + c] = 0.2126*d_in[r*w + c].x + 0.7152*d_in[r*w + c].y + 0.0722*d_in[r*w + c].z;
}
...
__global__
void horizSobelKernel(unsigned char *d_out, const unsigned char *d_in,
                   const float *d_filter, int w, int h) {
  const int c = threadIdx.x + blockDim.x * blockIdx.x;
  const int r = threadIdx.y + blockDim.y * blockIdx.y;
  if ((c >= w) || (r >= h)) return;
  ...
  extern __shared__ unsigned char hs_block[];
}
...
void intensityParallel(uchar4 *arr, unsigned char* oarr, int w, int h) {
  const dim3 blockSize(TX, TY);
  const dim3 gridSize(divUp(w, TX), divUp(h, TY));
  const size_t smSz = ((TX+2*RAD)*(TY+2*RAD)+(TX*TY))*sizeof(uchar4);
  ...
  intensityKernel<<<gridSize, blockSize, smSz >>>(d_out, d_in, w, h);
  cudaMemcpy(oarr, d_out, w*h*sizeof(unsigned char), cudaMemcpyDeviceToHost);
  cudaFree(d_in);
  cudaFree(d_out);
}
...
void horizSobelParallel(unsigned char *arr, int w, int h) {
  const int fltSz = 2*RAD + 1;
  const float filter[9] = {-1.0, 0.0, 1.0,
                           -2.0, 0.0, 2.0,
                           -1.0, 0.0, 1.0};
  unsigned char *d_in = 0, *d_out = 0;
  float *d_filter = 0;
  ...
  const dim3 blockSize(TX, TY);
  const dim3 gridSize(divUp(w, TX), divUp(h, TY));
  const size_t smSz = ((TX+2*RAD)*(TY+2*RAD)+(TX*TY))*sizeof(unsigned char);
  horizSobelKernel<<<gridSize, blockSize, smSz >>>(d_out, d_in, d_filter, 
                                               w, h);
  cudaMemcpy(arr, d_out, w*h*sizeof(unsigned char), cudaMemcpyDeviceToHost);
  cudaFree(d_in);
  cudaFree(d_out);
  cudaFree(d_filter);
}